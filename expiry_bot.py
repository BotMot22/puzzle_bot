...
def evaluate_candidate(question, clob_ask, clob_bid, hours_left):
    ...
    """
    # ── Filter 0: Stocks/crypto only ──
    if STOCKS_ONLY and not has_price_feed(question):
...
def print_banner():
    ...
    print(f"  Bet: ${BET_SIZE:.2f} | Scan: {SCAN_INTERVAL}s | Soccer: blocked")
...
def run():
    ...
    while True:
        try:
            ...
            # ── Wait: poll resolve/redeem every 5 min between scans ──
            next_scan = now_ts + SCAN_INTERVAL
            while time.time() < next_scan:
                sleep_secs = min(RESOLVE_INTERVAL, next_scan - time.time())
                if sleep_secs <= 0:
                    break
                time.sleep(sleep_secs)

                # Resolve + redeem between scans if pending trades exist
                if state["pending"]:
                    has_expired = any(
